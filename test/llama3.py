...
class Llama3Model:
    ...
    def load_weights(self):
        # Edge 需要全量权重用于 Prefill 计算
        full_weights = torch.load(
            os.path.join(MODEL_PATH, "original/consolidated.00.pth"), 
            map_location=DEVICE, weights_only=True
        )
        self.weights = {k: v.to(device=DEVICE, dtype=torch.bfloat16) for k, v in full_weights.items()}
    ...
    def run(self):
        ...
        time_to_first_token = time.time() - prefill_start_time
        print(f"Time to first token: {time_to_first_token:.4f} seconds")
        print(f"Next token: {tokenizer.decode(next_token.cpu().numpy())}")

        # --- Phase 2: Decode ---
        token_gen_times = []

        for _ in range(OUTPUT_TOKEN_NUM): # 生成 128 个词
            t_token_start = time.time()
            token_tensor = next_token.view(1, 1)
            h = torch.nn.functional.embedding(token_tensor, self.weights["tok_embeddings.weight"]).to(torch.bfloat16)

            for layer in range(self.config.n_layers):
                h = self.forward_layer_decode(h, layer, start_pos=seq_len)
